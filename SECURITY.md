...
- The reference interpreter (`src/mklang/`): engine, adapters, CLI, MCP
  ...
  - The MCP server is **read-only to disk by design**: it can author, validate
    (`check`) and run inline machines, but exposes no persist/write tool — a
    headless host gains no filesystem authority ([ADR 0011](./docs/adr/0011-mcp-server-surface.md),
