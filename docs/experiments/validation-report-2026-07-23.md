...
| C3 | **CONFIRMED via recorded runs; Anthropic still blocked** | 2026-07-23: **1.0 per machine**. 2026-07-24 (1.0.1): pooled **0.917**; `severity_escalate` **0.667** (one DeepSeek auto vs page). Release-gate single machine still **1.0**. Anthropic: **key absent** in maintainer env and GH secrets. | Coarse routing still solid on three machines; escalate contestability is real. | #60 — set `ANTHROPIC_API_KEY` (local + GH secrets) + credit. |
...
| C3 | Run one Anthropic four-machine pass (~30k tok) to close the three-provider gap. | XS | Blocked — key empty locally **and** missing from GH Actions secrets |
