...
4. **Examples** — add/adjust a machine in `examples/` that exercises the feature.
5. **Tests** — deterministic coverage with `MockLLM` in `tests/`; keep `ruff` clean.
6. **Docs** — `README.md`, `docs/patterns.md`, `CHANGELOG.md`, and `ROADMAP.md`.

A change to the **interpreter only** (no language change) skips steps 1–2.

## Design decisions (ADRs)

Non-trivial or contentious decisions get a short ADR in `docs/adr/NNNN-title.md`
(Context / Decision / Consequences). See the existing five for the format. Reference
the ADR in your PR.
